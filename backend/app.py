...
else:from flask import Flask, request, jsonify
from flask_cors import CORS
import os
import json
from datetime import datetime
...
if NEON_DATABASE_URL:
    print(f"[Backend] ✓ NEON_DATABASE_URL configured (length: {len(NEON_DATABASE_URL)})")
else:
    print("[Backend] ✗ WARNING: No NEON_DATABASE_URL found in environment")
...
def get_db_connection():
    ...
    try:
        if not NEON_DATABASE_URL:
            raise Exception("NEON_DATABASE_URL environment variable not set. Please configure your Neon database connection.")
        ...
        print(f"[Backend] NEON_DATABASE_URL set: {bool(NEON_DATABASE_URL)}")
        raise
...
if __name__ == '__main__':
    try:
        ...
        print(f"[Backend]   NEON_DATABASE_URL={NEON_DATABASE_URL}")
        import traceback
        traceback.print_exc()
